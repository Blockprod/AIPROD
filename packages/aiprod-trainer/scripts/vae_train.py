...
def main(
    ...
    trainer_config = VAETrainerConfig(
        ...
        weight_decay=train_raw.get("weight_decay", 1e-2),
        batch_size=train_raw.get("batch_size", 4),
        num_epochs=train_raw.get("num_epochs", train_raw.get("epochs", 10)),
        warmup_steps=train_raw.get("warmup_steps", 1000),
        beta_kl=raw.get("loss", {}).get("beta_kl", 0.1),
        lambda_perceptual=raw.get("loss", {}).get("lambda_perceptual", 1.0),
        use_perceptual_loss=raw.get("loss", {}).get("use_perceptual_loss", True),
        lambda_spectral=raw.get("loss", {}).get("lambda_spectral", 1.0),
        gradient_accumulation_steps=train_raw.get("gradient_accumulation_steps", 1),
        max_grad_norm=train_raw.get("max_grad_norm", 1.0),
        mixed_precision=train_raw.get("mixed_precision", "bf16"),
