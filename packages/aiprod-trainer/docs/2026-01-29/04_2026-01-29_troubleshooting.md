...
This guide covers common issues and solutions when training with the AIPROD trainer.

## 🔧 VRAM and Memory Issues

Memory management is crucial for successful training with AIPROD.

> [!TIP]
> For GPUs with 32GB VRAM, use the pre-configured low VRAM config:
> [`configs/AIPROD2_av_lora_low_vram.yaml`](../configs/AIPROD2_av_lora_low_vram.yaml)
...
### Issue: "No module named 'AIPROD_trainer'" Error
...
**Solution:**
AIPROD requires local model paths. URLs are not supported:
...
**Solution:**
AIPROD requires the number of frames to satisfy `frames % 8 == 1`:
...
2. **Use torch.compile** via Accelerate:
   ...
   uv run accelerate launch --config_file configs/accelerate/ddp_compile.yaml \
     scripts/train.py configs/AIPROD2_av_lora.yaml
   ```
...
4. **Check caption quality:**
   ...
   AIPROD prefers long, detailed captions that describe both visual content and audio (e.g., ambient sounds, speech,
   music).
...
Join our [community Discord server](https://discord.gg/AIPRODplatform)
