...
| Component                                                                               | Purpose                                                      |
|-----------------------------------------------------------------------------------------|--------------------------------------------------------------|
| [`TrainingStrategyConfigBase`](../src/AIPROD_trainer/training_strategies/base_strategy.py) | Base class for strategy configuration (Pydantic model)       |
| [`TrainingStrategy`](../src/AIPROD_trainer/training_strategies/base_strategy.py)           | Abstract base class defining the strategy interface          |
| [`ModelInputs`](../src/AIPROD_trainer/training_strategies/base_strategy.py)                | Dataclass containing prepared inputs for the transformer     |
| [`Modality`](../../AIPROD-core/src/AIPROD_core/model/transformer/modality.py)                 | AIPROD-core dataclass representing video or audio modality data |
...
Create a new file for your strategy (e.g., `src/AIPROD_trainer/training_strategies/inpainting.py`):
...
**1. Update [`src/AIPROD_trainer/training_strategies/__init__.py`](../src/AIPROD_trainer/training_strategies/__init__.py):**

```python
# Add import for your strategy
from AIPROD_trainer.training_strategies.inpainting import InpaintingConfig, InpaintingStrategy
...
**2. Update [`src/AIPROD_trainer/config.py`](../src/AIPROD_trainer/config.py):**

```python
# Add import
from AIPROD_trainer.training_strategies.inpainting import InpaintingConfig
...
```yaml
# configs/AIPROD2_inpainting_lora.yaml
...
The `Modality` dataclass (from AIPROD-core) represents a single modality's data:
...
1. **Verify your training configuration is valid:**
   ...
   with open('configs/AIPROD2_inpainting_lora.yaml') as f:
       config = AIPRODTrainerConfig(**yaml.safe_load(f))
...
2. **Test strategy instantiation:**
   ```bash
   uv run python -c "
   from AIPROD_trainer.training_strategies import get_training_strategy
   from AIPROD_trainer.training_strategies.inpainting import InpaintingConfig
...
3. **Run a short training test:**
   ```bash
   uv run python scripts/train.py configs/AIPROD2_inpainting_lora.yaml
   ```
...
- [AIPROD-core Documentation](../../AIPROD-core/README.md) - Core model components
...
|------------------------------------------------------------------------------------|------------|------------------------------------------------|
| [`TextToVideoStrategy`](../src/AIPROD_trainer/training_strategies/text_to_video.py)   | Simple     | First-frame conditioning, optional audio       |
| [`VideoToVideoStrategy`](../src/AIPROD_trainer/training_strategies/video_to_video.py) | Medium     | Reference video concatenation, split loss mask |
