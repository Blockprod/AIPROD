...
The main configuration class is [`AIPRODTrainerConfig`](../src/AIPROD_trainer/config.py), which includes the following
sub-configurations:
...
- 📄 [Audio-Video LoRA Training](../configs/AIPROD2_av_lora.yaml) - Joint audio-video generation training
- 📄 [Audio-Video LoRA Training (Low VRAM)](../configs/AIPROD2_av_lora_low_vram.yaml) - Memory-optimized config for 32GB
...
The AIPROD transformer has separate attention and feed-forward blocks for video and audio, as well as cross-attention
...
wandb:
  ...
  project: "AIPROD-trainer"     # W&B project name
