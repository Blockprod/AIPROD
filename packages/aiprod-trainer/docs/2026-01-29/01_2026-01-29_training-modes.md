...
AIPROD supports joint audio-video generation. You can train LoRA adapters that affect both video and audio output:
...
- 📄 [Audio-Video LoRA Training](../configs/AIPROD2_av_lora.yaml)
...
> [!IMPORTANT]
> Full fine-tuning of AIPROD requires multiple high-end GPUs (e.g., 4-8× H100 80GB) and distributed
...
- 📄 [IC-LoRA Training](../configs/AIPROD2_v2v_ic_lora.yaml) - Video-to-video transformation training
...
After training, use the [`AIPROD-pipelines`](../../AIPROD-pipelines/) package for production inference with your trained
LoRAs:
...
All pipelines support loading custom LoRAs via the `loras` parameter. See the [`AIPROD-pipelines`](../../AIPROD-pipelines/)
package
